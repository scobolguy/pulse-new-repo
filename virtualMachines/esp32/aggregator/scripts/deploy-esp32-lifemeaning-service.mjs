...
const LOCAL_PASCAL = path.resolve(process.cwd(), 'data', 'lifemeaning-service.pas');
const LOCAL_ROUTER_RULES = path.resolve(process.cwd(), '..', 'pcode', 'lifemeaning-router-rules.generated.json');
const LOCAL_DATA_MAPPINGS = path.resolve(process.cwd(), '..', 'pcode', 'lifemeaning-data-mappings.generated.json');
const REMOTE_ROUTER_RULES = '/lmr.json';
const REMOTE_DATA_MAPPINGS = '/lmm.json';
...
async function compilePascalish() {
  await execFileAsync(process.execPath, [
    ...
    '--artifact-out', path.resolve(process.cwd(), '..', 'pcode', 'lifemeaning-compiled.artifact.json')
  ], {
    cwd: process.cwd(),
    windowsHide: true,
...
async function main() {
  ...
  assert.equal(String(first.outputQueue || ''), 'lifemeaning.out', 'unexpected output queue');
  assert.equal(String(first.message || ''), '42', 'unexpected output message');

  console.log(JSON.stringify({
    status: 'ok',
    host: ESP32_HOST,
    serviceId: result.serviceId || 'LifeMeaning',
    delivery: {
...
main().catch((errorValue) => {
  console.error('[deploy-esp32-lifemeaning-service] failed:', errorValue?.message || String(errorValue));
  process.exitCode = 1;
});
