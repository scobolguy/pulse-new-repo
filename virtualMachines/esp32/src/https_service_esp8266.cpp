// ESP8266 TLS route management.
// Intentionally separate from ESP32 route/runtime implementation.
...
String makeSessionId() {
    const uint32_t r = static_cast<uint32_t>(random(0x7FFFFFFF));
    return String("enr-") + String((unsigned long)millis()) + String("-") + String((unsigned long)r, HEX);
}

String getDeviceId() {
    return String("esp8266-") + String((unsigned long)ESP.getChipId(), HEX);
}
...
String generateBootstrapToken() {
    ...
    for (size_t i = 0; i < TLS_BOOTSTRAP_TOKEN_BYTES; ++i) {
        token[i] = static_cast<uint8_t>(random(256));
    }
...
String loadOrCreateBootstrapToken() {
    ...
    if (gBootstrapToken.length() == 0) {
        ...
        Serial.printf("[TLS-ESP8266] Generated bootstrap token: %s\n", gBootstrapToken.c_str());
    }
...
void loadTlsState() {
    ...
    if (!LittleFS.exists(kTlsStatePath)) return;
...
void registerHttpsTlsRoutes(AsyncWebServer& srv) {
    ...
    Serial.println("[TLS-ESP8266] TLS management routes registered");
}
